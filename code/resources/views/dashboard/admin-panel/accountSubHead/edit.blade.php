            ...
            <div class="row">
                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="x_panel">
                        ...
                        <div class="x_content">
                            <br />
                            <form id="demo-form2" method="post" enctype="multipart/form-data" action="{{ route('account-sub-head.update', [$accountSubHeads->id] ) }}" data-parsley-validate class="form-horizontal form-label-left">
                                <input type="hidden" name="_method" value="PUT">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">

                                <div class="form-group">
                                    <label class="control-label col-md-3 col-sm-3 col-xs-12" for="name"> Account Head Name <span class="required">*</span>
                                    </label>
                                    <div class="col-md-8 col-sm-6 col-xs-12">
                                        <input type="text" id="name" required="required" value="{{ $accountSubHeads->name }}" name="name" class="form-control col-md-7 col-xs-12">
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-md-8 col-sm-6 col-xs-12">
                                        <select class="form-control" id="head_id" required="required" name="head_id">
                                            ...
                                            @foreach( $accountHeads as $accountHead )
                                                <option value="{{ $accountHead->id }}" {{ ($accountHead->id == $accountSubHeads->head_id) ? 'selected' : '' }}> {{ $accountHead->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
